...
func (p *Parser) registerPrefixFns() {
	...
	p.registerPrefix(token.MONEY, p.parseMoneyLiteral)
...
func (p *Parser) parseExpression(precedence int) ast.Expression {
	...
	for !p.peekTokenIs(token.BANG) && precedence < p.peekPrecedence() {
...
func (p *Parser) parseMoneyLiteral() ast.Expression {
	lit := &ast.MoneyLiteral{Token: p.curToken}
...
func (p *Parser) parseIfExpression() ast.Expression {
	...
	for p.peekTokenIs(token.ELSE_IF) {
		p.nextToken()
		p.nextToken()
		altCondition := p.parseExpression(LOWEST)
		if !p.expectPeek(token.LBRACE) {
			return nil
		}
		altConsequence := p.parseBlockStatement()
		expression.Alternatives = append(expression.Alternatives, &ast.IfAlternative{
			Condition:   altCondition,
			Consequence: altConsequence,
		})
	}
